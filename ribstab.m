...
maxeigvc = [];
Vmaxeig = [];
for i = 1:nx % for each wavelength:
    lxpi = lxp(i); 
    alp = 2*pi*Rt/lxpi; % extracts wavenumber in channel units
    ii = (1:size(eigvals,1)); % number of eigenvalues
    eevl = squeeze(eigvals(ii,i)); % remove dimensions length 1
    eevc = squeeze(eigvecs(:,ii,i));
    if isempty(eevl) % no eigenvalues
        maxeigvl(i) = 0;
        maxeigvc(:,i) = 0*(1:size(y,1));
        Vmaxeig (i) = 0;
    elseif isempty(find(max(imag(eevl)))) % stable mode
        indmax = 1;
        ...
        maxeigvl(i) = eevl(indmax);
        maxeigvc(:,i) = eevc(:,indmax);
        Vmaxeig(i) = real(eevl(indmax))/alp/ut; % velocity in channel units
    end
end
