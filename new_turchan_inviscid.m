function d = turchan_inviscid(alp,D0,D1,D2,D3,D4,U,Rt,iKw,nut,iSw,Re,lhq,lsq,L);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                    %
% Function to create Orr-Sommerfeld matrices using Chebyshev         %
% pseudospectral discretization for channel flow.                    %
%                                                                    %
% INPUT                                                              %
%                                                                    %
...
nutppp=nut(:,4); nutpp=nut(:,3); nutp=nut(:,2); nut=nut(:,1);
...
A11 =  A11 + i*(nut*unos).*(D4 - 2*ak2*D2 + (ak2^2)*D0);
A11 =  A11 + i*(nutp*unos).*( 2*D3 - 2*ak2*D1);  %%% viscous, nutp
A11 =  A11 + i*(nutpp*unos).*( D2 + ak2*D0);
...
% 
A11t= (-alp*Uy(1)-i*iKw)*D0(1,:) ;
A11t= A11t + alp*U(1)*D1(1,:); % U =/= 0 added
% % A11t= A11t -i*1/Re*(D3(1,:)-ak2*D1(1,:));
A11t= A11t +i*nut(1)*(D3(1,:)-ak2*D1(1,:));
A11t= A11t +i*nutp(1)*(D2(1,:)+ak2*D0(1,:));
%With shear 
A11t= A11t + i*D2(1,:)*L;

A11b= (-alp*Uy(end)+i*iKw)*D0(end,:) ;
A11b= A11b + alp*U(end)*D1(end,:);
% % A11b= A11b -i*1/Re*(D3(end,:)-ak2*D1(end,:));
A11b= A11b +i*nut(end)*(D3(end,:)-ak2*D1(end,:));
A11b= A11b +i*nutp(end)*(D2(end,:)+ak2*D0(end,:));
% With shear
A11b= A11b + i*D2(end,:)*L;
